...
// Every canonical IANA zone accepted by the backend @ValidTimeZone validator
// (continent/area prefix or UTC/GMT). Used only when Intl.supportedValuesOf
// is unavailable; modern runtimes return the same set dynamically.
const FALLBACK_TIME_ZONES = [
  ...
  "America/El_Salvador","America/Fort_Nelson","America/Fortaleza","America/Glace_Bay","America/Godthab",
  ...
  "Asia/Chita","Asia/Colombo","Asia/Damascus","Asia/Dhaka","Asia/Dili","Asia/Dubai",
  ...
  "Asia/Karachi","Asia/Katmandu","Asia/Khandyga","Asia/Krasnoyarsk","Asia/Kuala_Lumpur","Asia/Kuching",
  ...
  "Europe/Tirane","Europe/Ulyanovsk","Europe/Vaduz","Europe/Vatican","Europe/Vienna","Europe/Vilnius",
  "Europe/Volgograd","Europe/Warsaw","Europe/Zagreb","Europe/Zurich","Indian/Antananarivo","Indian/Chagos",
  ...
  "Pacific/Chatham","Pacific/Easter","Pacific/Efate","Pacific/Enderbury","Pacific/Fakaofo","Pacific/Fiji",
...
function formatOffset(zone: string): string {
  try {
    ...
    return `(${offset.replace(/^GMT/, "UTC")}) ${zone}`;
  } catch {
    return zone;
  }
}

function offsetMinutes(label: string): number | null {
  const match = label.match(/^\(UTC([+-]\d{1,2}(?::\d{2})?)?\)/);
  if (!match) return null;
  if (match[1] === undefined) return 0;
  const sign = match[1].startsWith("-") ? -1 : 1;
  const [hours, minutes] = match[1].replace(/^[+-]/, "").split(":");
  return sign * (Number(hours) * 60 + Number(minutes ?? 0));
}

function compareOffset(a: string, b: string): number {
  const minutesA = offsetMinutes(a);
  const minutesB = offsetMinutes(b);
...
export function getTimeZoneOptions(): TimeZoneOption[] {
  ...
  cachedOptions = zones
    .map((zone) => ({ value: zone, label: formatOffset(zone) }))
    .sort((a, b) => {
      const offsetDiff = compareOffset(a.label, b.label);
...
export function isValidTimeZone(zone: string): boolean {
  return getTimeZoneOptions().some((option) => option.value === zone);
}
...
function resolveDefaultTimeZone(): string {
  try {
    ...
    // Fall back to UTC when the runtime cannot resolve a time zone.
  }
  return "UTC";
}

export const DEFAULT_TIME_ZONE: string = resolveDefaultTimeZone();
